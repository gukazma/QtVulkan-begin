...
void QuadRenderer::initResources()
{
    ...
    vk::DeviceSize uniformAllocSize =
        aligned(16 * sizeof(float), limits.minUniformBufferOffsetAlignment);
    ...
    vk::MemoryAllocateInfo uniformMemAllocInfo(uniformMemReq.size,
    ...
    uint8_t* uniformBufferMemPtr =
        (uint8_t*)device.mapMemory(uniformDevMemory_, 0, uniformMemReq.size);
    QMatrix4x4 identify;
    ...
    vk::PipelineVertexInputStateCreateInfo vertexInputState(
        {}, 1, &vertexBindingDesc, 2, vertexAttrDesc);
    piplineInfo.pVertexInputState = &vertexInputState;

    vk::PipelineInputAssemblyStateCreateInfo vertexAssemblyState(
...
void QuadRenderer::startNextFrame()
{
    ...
    uint8_t* uniformMatrixMemPtr =
        (uint8_t*)device.mapMemory(uniformDevMemory_,
